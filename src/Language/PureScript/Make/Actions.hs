...
buildMakeActions outputDir filePathMap foreigns usePrefix =
  ...
  targetFilename mn = \case
    ...
    Erl -> outputFilename mn $ T.unpack (atomModuleName mn PureScriptModule <> ".erl")
  ...
  codegenErl :: CF.Module CF.Ann -> Environment -> Externs -> SupplyT Make ()
  codegenErl m env exts = do
  ...
  codegen m env exts = do
    ...
    when (S.member Erl codegenTargets) $ codegenErl m env exts
